...
"""
twins: a list of the node's twins from previous rounds.
deltaTwins: a list of the node's twins during delta consecutive rounds.
"""
def receiveMsg(round, p, n_socket, myIp, nbN, tab, twins, deltaTwins, delta):
    ...
    if(round < p):
        ...
        for key, val in compt.items():
            if(val == myNeighbors and nbNeighbors[key] == myNeighbors and int(key) != int(myIp)):
                ...
                if(twins[key] == delta):
                    #Debuging message
                    #print("I am ", myIp, " I add ", key, " in round ", round-p+1-delta)
                    deltaTwins.append([int(key), (round-p+1)-delta])
                    twins[key] = 0
            else:
                twins[key] = 0

            
def noeud(id, p, delta):
    ...
    while(round != 2*p):
        ...
        recevThread = threading.Thread(target=receiveMsg, args=(round, p, n_socket, programme_id, len(listeNeighbors), tab, twins, deltaTwins, delta))
        ...
        if(round == 2*p):
            #Debuging message
            print("I am ", id, "my delta twins are : ", deltaTwins)
...
def manager(ip, idD, p, delta):
    ...
    while i < nbNodes:
        ...
        tabProcess.append(multiprocessing.Process(target=noeud, args=(setNodes[i], p, delta)))
        i+= 1
