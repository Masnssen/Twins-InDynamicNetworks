...
#The "dTwins" dictionary is used to associate each node with its corresponding minimum deleted twin.
def receiveMsg(round, p, n_socket, myIp, nbN, tab, dTwins):
    ...
    if(round < p):
        ...
        for key, val in compt.items():
            dTwin = myNeighbors + nbNeighbors[key] - 2 * val
            if key in dTwins:
                if dTwin < dTwins[key]:
                    dTwins[key] = dTwin
            else:
...
def node(id, p):
    ...
    while(round != 2*p):
        ...
        recevThread = threading.Thread(target=receiveMsg, args=(round, p, n_socket, programme_id, len(listeNeighbors), tab, dTwins))
        ...
        if(round == 2*p):
            ...
            twins = dict()
            for key, val in dTwins.items():
                if(val in twins):
                    twins[val].append(key)
                else:
                    twins[val] = [key]
            
            for key, val in twins.items():
                print("I am ", id, " my ", int(key), "-twins are ", val)
...
def manager(ip, idD, p):
    ...
    while i < nbNodes:
        ...
        tabProcess.append(multiprocessing.Process(target=node, args=(setNodes[i], p)))
        i+= 1
